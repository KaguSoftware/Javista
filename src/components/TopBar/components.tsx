...
export function TopBar({
  ...
  return (
    <div className="shrink-0 px-4.5 pt-2 pb-2.5 flex items-center justify-between gap-2.5 bg-bg border-b-2 border-ink/15">
      <button type="button" onClick={onTopClick} className="flex items-center gap-2.5 cursor-pointer border-0 bg-transparent p-0">
        ...
        <div>
          <div className="font-bowlby text-[18px] leading-[0.9] text-ink tracking-[-0.5px] uppercase">
            {brandMain}
            <span className="text-orange">{brandAccent}</span>
          </div>
          <div className="text-[8px] tracking-[0.28em] font-bold text-ink/60 uppercase mt-0.5">
            {brandSub}
          </div>
        </div>
      </button>
